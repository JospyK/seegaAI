...
class AI(Player):
    ...
    def best_entry_position_v2(self, state):
        for i in range(4):
            for j in range(4):
                if SeegaRules.is_legal_move(state, SeegaAction(action_type=SeegaActionType.ADD, to=(i, 0)), self.position):
                    return self.buildSeegaAction([(i, 0)])
                if SeegaRules.is_legal_move(state, SeegaAction(action_type=SeegaActionType.ADD, to=(0, j)), self.position):
                    return self.buildSeegaAction([(0, j)])
                if SeegaRules.is_legal_move(state, SeegaAction(action_type=SeegaActionType.ADD, to=(i, 4)), self.position):
                    return self.buildSeegaAction([(i, 4)])
                if SeegaRules.is_legal_move(state, SeegaAction(action_type=SeegaActionType.ADD, to=(4, j)), self.position):
